...
const crashLogger = require('pome-logger').getLogger('crash-log', __filename);
const adminLogger = require('pome-logger').getLogger('admin-log', __filename);
const admin = require('pome-admin');
...
class Master
{
	...
	start(cb)
    {
		...
		this.masterConsole.start( (err) =>
        {
			...
			moduleUtil.startModules(self.modules, (err) =>
            {
				if (err)
                {
