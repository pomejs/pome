const async = require('async'),
	...
	logger = require('pome-logger').getLogger('pome', __filename);
...
class AppUtil
{
	...
	static loadDefaultComponents(app)
    {
        ...
        {
			app.load(pome.master, app.get('masterConfig'));
		}
		else
        {
			app.load(pome.proxy, app.get('proxyConfig'));
			if (app.CurServer.port)
            {
				app.load(pome.remote, app.get('remoteConfig'));
			}
			if (app.isFrontend())
            {
				app.load(pome.connection, app.get('connectionConfig'));
				app.load(pome.connector, app.get('connectorConfig'));
				app.load(pome.session, app.get('sessionConfig'));
                // compatible for schedulerConfig
				if (app.get('schedulerConfig'))
                {
					app.load(pome.pushScheduler, app.get('schedulerConfig'));
				}
				else
                {
					app.load(pome.pushScheduler, app.get('pushSchedulerConfig'));
				}
			}
			app.load(pome.backendSession, app.get('backendSessionConfig'));
			app.load(pome.channel, app.get('channelConfig'));
			app.load(pome.server, app.get('serverConfig'));
		}
		app.load(pome.monitor, app.get('monitorConfig'));
	}
...
{
	if (process.env.POME_LOGGER !== 'off')
    {
