...
(function() {
  ...
  pome.init = function(params, cb){
    ...
    socket.on('connect', function(){
      console.log('[pomeclient.init] websocket connected!');
      if (cb) {
        cb(socket);
      }
    });
    ...
    socket.on('message', function(data){
      ...
      if(data instanceof Array) {
        processMessageBatch(pome, data);
      } else {
        processMessage(pome, data);
      }
    });
  ...
  var processMessage = function(pome, msg) {
    ...
    if(msg.id) {
      ...
      if(typeof cb !== 'function') {
        console.log('[pomeclient.processMessage] cb is not a function for request ' + msg.id);
        return;
      }
    ...
    function processCall(msg) {
      ...
      if(!!route) {
        if (!!msg.body) {
          ...
          pome.emit(route, body);
        } else {
          pome.emit(route,msg);
        }
      } else {
          pome.emit(msg.body.route,msg.body);
      }
    }
  };
